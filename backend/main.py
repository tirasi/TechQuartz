...
async def signup(data: dict):
    ...
    if any(u.get("email") == email or u.get("phone") == phone for u in db["users"]):
...
async def login(data: dict):
    ...
    user = next((u for u in db["users"] 
                 if u.get("email") == identifier or u.get("phone") == identifier), None)
...
async def complete_profile(data: dict, authorization: str = Header(None)):
    current_user = get_current_user(authorization)
    db = load_db()
